...
function [error, cost, w, BT_P, BT_N, norms] = AFTRL_reg(data, model, opts)
    AT = data.A';
    b = data.b;
    [d, n] = size(AT);
    error = zeros(n, 1);
    ...
    for t=1: floor(0.8*n)
        ...
        BPa = BT_P' * a;
        BNa = BT_N' * a;
        sclar =  BPa' * BPa - BNa' * BNa;%w' * a + BPa' * BPa - BNa' * BNa;
        %if mod(t,100)==0
		%	display(sclar);
		%end
	...
	for t=floor(n*0.8)+1:n
		...
		sclar = BPa' * BPa - BNa' * BNa;%w' * a + BPa' * BPa - BNa' * BNa;
        if t>floor(n*0.8)+1
            test_error(t-floor(n*0.8)) = test_error(t-floor(n*0.8)-1) + (sclar-b(t))^2;
        end
	end
	display(test_error(floor(0.2*n))/floor(0.2*n));
    avg_loss_dir = strcat('E:\Researchs\OCFM/loss_ccfm_aftrl_100k');
    save(avg_loss_dir,'error');
    time_dir = strcat('E:\Researchs\OCFM/time_ccfm_aftrl_100k');
    save(time_dir,'time');
    perf_dir = strcat('E:\Researchs\OCFM/perf_ccfm_aftrl_100k');
    save(perf_dir,'perf');
    %pred_dir = strcat('E:\Researchs\OCFM/pred_ccfm_aftrl_100k_norm',num2str(r));
